...
class Dictionary():
    ...
    __first_meet : str = '''А тебя я не знаю... Ты, значится, {{full_name}}!
А я Шут. Ромашковый.🤡 Я работаю на мобильных разработчиков в их мобильном подвале.
Дай-ка я на тебя взгляну...\nИзмерим твой {{pencil}}... Ого! Вот это питон! {{length}}см
{{custom_title}}'''
    
    __user_information : str = "🔍 {{tg_name}} {{custom_title}} Имеет {{pencil_accu}} длинной {{length}}!"
    ...
    ###------------------------------------------------------------
    ###Описание интерактивного действия изменения размера
    ###------------------------------------------------------------
...
'''От страха его {{pencil}} уменьшился на {{length}}''',   

'''📏«Краткость сестра таланта!». - С этими словами''' 
'''Шут уменьшает {{pencil_accu}} на {{length}} совершенно нечестным способом! {{tg_name}} в слезах! ''',

'''📏Шут негодует: «Работал бы лучше, чем тут ерундой заниматься! ''' 
'''Выписываю тебе штраф в размере... {{length}}, давай оттяпывай»''',

'''📏Шут раздает получку, {{tg_name}} депремирован на {{length}}! {{pencil_accu}}''',
...
'''{{tg_name}} пользовался советом три дня, но {{pencil}} сморщился на {{length}}''',
    ...
    member_names:List[List] = [
        ...
        ["пипиролька", "пипирольки", "пипирольке","пиперольку","пипиролькой",""],
    ...
    def first_meet(self, full_name:str, length:int,  custom_title: Optional[str]) -> str:  
    ...
    def user_information(self,user:UserModel) -> str:
        return tp.text_replacement(self.__user_information,
                                   {"tg_name": user.tg_name, 
                                    **self.random_member(),
                                    "custom_title" : code(f'[{user.custom_title}]') if user.custom_title is not None else '',
    ...
    def length_change(self, tg_name:str, length_change:int) -> str:
        params: Dict = {**self.random_member(), 
                        ...
                        "tg_name": bold(tg_name)}
        
        if (length_change > 0):
            return tp.text_replacement(
                self.positive_length_change[random.randint(0, len(self.positive_length_change) - 1)],
                                       params)
        else:
            return tp.text_replacement(
                self.negative_length_change[random.randint(0, len(self.negative_length_change) - 1)],
                                       params)
    ...
    def length_wrapper(self, length:int, plus_visible:bool = True) -> str:
        return code(f'{"+" if (length > 0 and plus_visible) else ""}{length}см')
